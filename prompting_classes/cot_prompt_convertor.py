...
class CoTPromptConvertor(ZeroShotPromptConvertor):
    def __init__(self, *args, **kwargs):
        ...
        self.cot_tasks = config.prompt_config.cot_tasks
    ...
    def get_task_result(self, task_id):
        ...
        return prompt_allged_input + [{'role': 'assistant', 'content': alleged_output}]

    def convert_task_to_prompt(self, train, test):
        cot_prompts = []
        for task_id in self.cot_tasks:
            cot_prompts+= self.get_task_result(task_id)
        actual_message = super().convert_task_to_prompt(train,test)
        cot_prompts += actual_message
        return cot_prompts
        # total_cot = ''
    ...
    def __str__(self):
        return f'CoT_UsingNumbers={self.using_numbers}'
